// В ПРОЕКТЕ НЕ ИСПОЛЬЗУЕТСЯЮ А ЛИШЬ ДЕМОНСТРИРУЕТ СУТЬ РАБОТЫ С API
...
export const useRequestApi = ({url, options, isAutoLoad}) => {
  ...
  useEffect(() => {
    if (isAutoLoad) { //isAutoLoad - хотим, чтобы при инициализации выполнялся запрос
      makeRequest();
    }
  }, [])


  return [ // сигнатура как у useState (данные, метод для работы с данными)
    {
      isLoading,
      error,
      data,
